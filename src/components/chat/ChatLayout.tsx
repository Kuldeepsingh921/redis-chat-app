...
const ChatLayout = ({defaultLayout=[320,480]}:ChatLayoutProps) => {
  ...
  return (
    <ResizablePanelGroup direction='horizontal' className='h-full items-stretch bg-background rounded-lg' onLayout={(sizes:number[])=>{
        ...
        <ResizablePanel defaultSize={defaultLayout[0]} collapsedSize={8} collapsible={true} minSize={isMobile ? 0 : 24} maxSize={isMobile ? 8 : 30} onCollapse={()=>{
            setIsCollapsed(true)
...
document.cookie='react-resizable-panels:collapsed=false'
        ...
        <ResizablePanel defaultSize={defaultLayout[1]} minSize={30} > 
            <div className='flex justify-center items-center h-full w-full px-10'>
                ...
                <p className='text-muted-foreground text-center select-none'>Click on the chat to view the messages</p>
                </div>
            </div>
            <div></div>
        </ResizablePanel>
